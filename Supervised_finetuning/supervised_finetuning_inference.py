...
def sf_infer(org_heatmap, gt_masks_paths, dataset_class,data_loader, run_save_path):
    DEVICE = 'cuda'
    ...
    model.load_state_dict(torch.load( run_save_path + f"/best_model_{dataset_class}.pt"))
...
"""
    ...
    print("anomaly segmentations shape", anomaly_segmentations.shape )
    
    print("flattened anomaly segmentations shape", anomaly_segmentations_flat.shape )

    print("groundtruth masks shape", np.array(masks_gt).shape )


    #masks_gt = np.array(masks_gt).reshape(n,h,w)
