...
def assess_anomaly_maps(obj, anomaly_maps, map_type=None):
    auroc_seg,aupr_seg, threshold = mvtecad.segmentation_auroc(obj, anomaly_maps)
    ...
    if map_type == 'multi':
        ...
        auroc_det,aupr_det= mvtecad.detection_auroc(obj, anomaly_scores)
        print(f'new_score_calculation: {auroc_det:.3f}')
    
    anomaly_scores = anomaly_maps.max(axis=-1).max(axis=-1)
    auroc_det,aupr_det= mvtecad.detection_auroc(obj, anomaly_scores)
    return auroc_det, auroc_seg,aupr_det,aupr_seg, anomaly_scores, threshold
...
def eval_embeddings_NN_multiK(obj, embs64, embs32, embs16, NN=1):
    ...
    det_64, seg_64,aupr_det64,aupr_seg64,_, _ = assess_anomaly_maps(obj, maps_64_distribute)
    ...
    det_32, seg_32,aupr_det32,aupr_seg32, _, _ = assess_anomaly_maps(obj, maps_32_distribute)
    ...
    det_16, seg_16,aupr_det16,aupr_seg16, _, _ = assess_anomaly_maps(obj, maps_16_distribute)

    maps_sum = maps_64_distribute + maps_32_distribute + maps_16_distribute
    det_sum, seg_sum,aupr_det_sum,aupr_seg_sum, imgs_score, threshold = assess_anomaly_maps(obj, maps_sum, map_type='multi')

    maps_mult = maps_64_distribute * maps_32_distribute * maps_16_distribute
    det_mult, seg_mult,aupr_det_mult,aupr_seg_mult, _, _ = assess_anomaly_maps(obj, maps_mult, map_type='multi')

    return {
        'det_64': det_64,
        'seg_64': seg_64,
        'aupr_det64':aupr_det64,
        'aupr_seg64':aupr_seg64,

        'det_32': det_32,
        'seg_32': seg_32,
        'aupr_det32':aupr_det32,
        'aupr_seg32':aupr_seg32,

        
        'det_16': det_16,
        'seg_16': seg_16,
        'aupr_det16':aupr_det16,
        'aupr_seg16':aupr_seg16,


        'det_sum': det_sum,
        'seg_sum': seg_sum,
        'aupr_det_sum':aupr_det_sum,
        'aupr_seg_sum':aupr_seg_sum,

        'det_mult': det_mult,
        'seg_mult': seg_mult,
        'aupr_det_mult':aupr_det_sum,
        'aupr_seg_mult':aupr_seg_sum,
