...
DATASET_PATH = '/home/mayooran/MVTecAD/'
...
def detection_auroc(obj, anomaly_scores):
    ...
    # AUPR computation
    precision, recall, thresholds = precision_recall_curve(label, anomaly_scores)
...
def segmentation_auroc(obj, anomaly_maps):
    ...
    precision, recall, thresholds = precision_recall_curve(gt.flatten(), anomaly_maps.flatten())
    a = 2 * precision * recall
    b = precision + recall
    f1 = np.divide(a, b, out=np.zeros_like(a), where=b != 0)
    threshold_f1 = thresholds[np.argmax(f1)]
    
    fpr, tpr, thresholds = roc_curve(gt.flatten(), anomaly_maps.flatten())
    threshold_roc = thresholds[np.argmax(tpr - fpr)]
    
    threshold = threshold_f1*(2/3) + threshold_roc*(1/3)
    ...
    return auroc,aupr, threshold_f1
